...
export async function attendanceToken(room_id: string, attendance_ids: string[], class_start_time: number, class_end_time: number) {
...
async function jwtInit(): Promise<{ options: SignOptions, secretOrPrivateKey: Secret, secretOrPublicKey: Secret }> {
    ...
    case "PS512": {
        ...
        const secretOrPrivateKey = passphrase ? { key: privateKey, passphrase } : privateKey;
