export class RedisKeys {
    ...
    public static whiteboardEvents(roomId: string) {
        return { key: `${RedisKeys.room(roomId)}:whiteboard:events`, ttl: 3600 };
    }

    public static whiteboardState(roomId: string) {
        return { key: `${RedisKeys.room(roomId)}:whiteboard:state`, ttl: 3600 };
    }

    public static whiteboardPermissions(roomId: string, userId: string) {
        return { key: `${RedisKeys.room(roomId)}:${RedisKeys.user(userId)}:whiteboard:permissions`, ttl: 3600 };
    }
